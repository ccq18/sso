...
class SsoUserProvider implements Provider
{
    ...
    public function retrieveById($identifier)
    {
        ...
        $user=   resolve(\Ido\Tools\SsoAuth\AuthHelper::class)->getUserById($identifier);
        if(empty($user)){
            return null;
        }
    ...
    {
        $user = resolve(\Ido\Tools\SsoAuth\AuthHelper::class)->getUserByTicket($credentials['ticket']);
        // dd('user',$user);
        if(empty($user)){
            return null;
        }
